...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            logger.exception(e)
            m=await message.reply_text("🔎") 
            fek = await message.reply_photo(photo=NOR_IMG, text=cap, reply_markup=InlineKeyboardMarkup(btn))
            await m.delete()
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(300)
        ...
        fuk = await message.reply_photo(photo=NOR_IMG, text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
        await m.delete()
        try:
            if settings['auto_delete']:
                await asyncio.sleep(300)
